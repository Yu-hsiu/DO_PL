...
| 目錄 / 檔案 | 內容 | 性質 |
| :--- | :--- | :--- |
| `ourc/` | **OurC 直譯器(自實作)** — 完成 Project 1~4(詞法 / 語法 / 語意 / 求值 / 函式呼叫) | 本人作品 |
...
### Project 4:函式呼叫堆疊(已完成)

- **使用者函式**:定義後可呼叫;呼叫時建立獨立框架綁定參數。
- **作用域**:函式框架只見全域與自身參數/區域變數,看不到呼叫端區域變數(C 語意)。
- **回傳值**:`return` 以訊號帶值回呼叫點;無 return 則為 void。
- **遞迴**:支援(如 `fact(n)` 自呼叫);運算式中可巢狀呼叫(如 `fact(3) + sum(3)`)。
- **引數檢查**:數量不符回報執行期錯誤。

```text
第 2 句: => 15      sum(5)          // 迴圈累加
第 4 句: => 120     fact(5)         // 遞迴階乘
第 8 句: => 12      fact(3) + sum(3)
```

> 已知限制:grammar 的 `&` by-reference 參數目前以傳值處理。
...
- [x] **P4** 使用者函式呼叫堆疊(參數綁定、回傳值、遞迴)

四階段完成,OurC 為可執行的直譯器。同題目的公開參考實作:
[HouHou0925/C-plus-interpreter](https://github.com/HouHou0925/C-plus-interpreter)(做到 P3)。
